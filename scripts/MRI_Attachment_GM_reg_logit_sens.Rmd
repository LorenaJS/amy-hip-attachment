---
title: "MRI Attachment - Sensitivity regression analyses for Amy/Hip structure and attachment behaviours (transformed behavioural data)"
author: "LJS"
date: "2023-02-28"
...
Scale brain data and residualise brain against age at scan and gestational age at birth (these keep their original name for simplicity).

```{r}
Data[,c(4:24)] <- scale(Data[,c(4:24)])
Data <- umx_residualize(c("vol_hip_l", "vol_hip_r", "vol_amy_l", "vol_amy_r", "MD_hip_l", "MD_hip_r", "MD_amy_l", "MD_amy_r", "FA_hip_l", "FA_hip_r", "FA_amy_l", "FA_amy_r", "NDI_hip_l", "NDI_hip_r", "NDI_amy_l", "NDI_amy_r", "ODI_hip_l", "ODI_hip_r", "ODI_amy_l", "ODI_amy_r"), cov = c("age_scan", "GA_birth"), data = Data)
```
...
write.csv(Total.report, here("results/regression", "GM.reg.logit.sens.csv"), row.names = FALSE)
```

### Plot associations between MRI features and attachment behaviours

```{r, fig.fullheight=TRUE}
data <- read.csv(here("results/regression", "GM.reg.logit.sens.csv"), header=T, dec=".",sep=",")
...
ggsave(here("results/figures", "GM.reg.logit.sens.heatmap.pdf"), plot = g, dpi=300)
```

```{r}
markdownToHTML("MRI_Attachment_GM_reg_logit_sens.Rmd",output ="MRI_Attachment_GM_reg_logit_sens.html")
```
