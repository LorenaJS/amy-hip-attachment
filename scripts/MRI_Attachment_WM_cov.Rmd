---
title: "MRI Attachment - Covariate analyses for Amy/Hip structural connectivity and attachment behaviours"
author: "LJS"
date: "2023-02-28"
...
MRI_DTI <- read.csv(here("raw_data", "IDs_MRI_structural_connectivity.csv"), header=T, dec=".",sep=",")
...
Residualise brain against age at scan (these keep their original name for simplicity).

```{r}
Data$Ethnicity <- NULL
Data[,c(1:19)] <- scale(Data[,c(1:19)])

Data <- umx_residualize(c("FA_hip_l", "FA_hip_r", "FA_amy_l", "FA_amy_r", "MD_hip_l", "MD_hip_r", "MD_amy_l", "MD_amy_r", "NDI_hip_l", "NDI_hip_r", "NDI_amy_l", "NDI_amy_r", "ODI_hip_l", "ODI_hip_r", "ODI_amy_l", "ODI_amy_r"), cov = c("age_scan"), data = Data)
```
...
```{r}
targetdata=Data[,grep('HD_Raw|FNF_Raw|AA_Raw|FA_|MD_|NDI_|ODI_', colnames(Data))]
targetdata=as.data.frame(targetdata)
apply(targetdata,2,shapiro.test)
```

According to above: all behavioural variables and all MRI variables seem non-normally distributed.
Use histograms to double-check distribution of these variables visually.

```{r}
targetdata=Data[,grep("_Raw|FA_|MD_|NDI_|ODI_", colnames(Data))]
targetdata=as.data.frame(targetdata)
...
According to above: FNF_Raw, FA_hip_l, FA_hip_r, FA_amy_l, FA_amy_r, MD_hip_l, MD_hip_r, MD_amy_l, MD_amy_r, NDI_hip_l, and ODI_amy_l seem non-normally distributed.
...
```{r}
targetdata <- Data[,c(1:20)]

table_cov <- targetdata %>% tbl_summary(by = sex,
  label = NULL,
  statistic = list(c("FNF_Raw", "FA_hip_l", "FA_hip_r", "FA_amy_l", "FA_amy_r", "MD_hip_l", "MD_hip_r", "MD_amy_l", "MD_amy_r", "NDI_hip_l", "ODI_amy_l") ~ c("{median} ({min}, {max})"), 
                   c("HD_Raw", "AA_Raw", "NDI_hip_r", "NDI_amy_l", "NDI_amy_r", "ODI_hip_l", "ODI_hip_r", "ODI_amy_r") ~ c("{mean} ({sd})"), all_categorical() ~ "{n} ({p}%)"),
  digits = everything()~3) %>% 
  add_p(
  test = list(c("FNF_Raw", "FA_hip_l", "FA_hip_r", "FA_amy_l", "FA_amy_r", "MD_hip_l", "MD_hip_r", "MD_amy_l", "MD_amy_r", "NDI_hip_l", "ODI_amy_l") ~ "wilcox.test",
                   c("HD_Raw", "AA_Raw", "NDI_hip_r", "NDI_amy_l", "NDI_amy_r", "ODI_hip_l", "ODI_hip_r", "ODI_amy_r") ~ "t.test", all_categorical() ~ "fisher.test"),
  pvalue_fun = Pval_fun) %>% 
...
write.csv(table_cov, here("results/covariates", "WM.Cov.sex.table.csv"), row.names=FALSE)
```

Associations between maternal smoking and MRI/behavioural variables.

```{r}
targetdata <- Data[,c(1:19,28)]
...
write.csv(table_cov, here("results/covariates", "WM.Cov.smo.table.csv"), row.names = FALSE)
```

Associations between maternal alcohol and MRI/behavioural variables.

```{r}
targetdata <- Data[,c(1:19,29)]
...
table_cov <- as_tibble(table_cov)
colnames(table_cov) <- c("Characteristic", "None", "Any", "t-statistic", "p-value")
write.csv(table_cov, here("results/covariates", "WM.Cov.alc.table.csv"), row.names = FALSE)
...
write.csv(table_cov, here("results/covariates", "WM.Cov.dep.table.csv"), row.names = FALSE)
```

Associations between maternal antenatal anxiety and MRI/behavioural variables.

```{r}
targetdata <- Data[,c(1:19,31)]
...
table_cov <- as_tibble(table_cov)
colnames(table_cov) <- c("Characteristic", "Absence", "Presence", "t-statistic", "p-value")
write.csv(table_cov, here("results/covariates", "WM.Cov.anx.table.csv"), row.names = FALSE)

```

According to above, no variable met the definition of confounder and was thereby used as a covariate in further analyses.
...
```{r, out.width="50%"}
targetdata <- Data[,c(1:19, 22,23,26,27)]
...
data_spearman <- data_spearman %>% filter(
    X == "FA_hip_l" | X == "MD_hip_l" | X == "NDI_hip_l" | X == "ODI_hip_l" |  
    X == "FA_hip_r" | X == "MD_hip_r" | X == "NDI_hip_r" | X == "ODI_hip_r" |
    X == "FA_amy_l" | X == "MD_amy_l" | X == "NDI_amy_l" | X == "ODI_amy_l" |
    X == "FA_amy_r" | X == "MD_amy_r" | X == "NDI_amy_r" | X == "ODI_amy_r" | 
    X == "HD_Raw" | X == "FNF_Raw"| X == "AA_Raw" )


targetdata=Data[,grep("FNF_Raw|FA_|MD_|NDI_hip_l|ODI_amy_l|Maternal_age", colnames(Data))]
...
MA_spearman <- MA_spearman %>% filter(
    X == "FA_hip_l" | X == "MD_hip_l" | X == "NDI_hip_l" |  
    X == "FA_hip_r" | X == "MD_hip_r" | 
    X == "FA_amy_l" | X == "MD_amy_l" | X == "ODI_amy_l" | 
    X == "FA_amy_r" | X == "MD_amy_r" | 
    X == "FNF_Raw")
...
targetdata <- Data[,c(1:19, 22,23,26,27)]
targetdata=Data[,-grep("FNF_Raw|FA_|MD_|NDI_hip_l|ODI_amy_l|Maternal_age", colnames(targetdata))]
...
MA_pearson <- MA_pearson %>% filter(
    X == "ODI_hip_l" |  
    X == "NDI_hip_r" | X == "ODI_hip_r" |
    X == "NDI_amy_l" | 
    X == "NDI_amy_r" | X == "ODI_amy_r" | 
    X == "HD_Raw" | X == "AA_Raw" )

data <- rbind(data, MA_pearson)

kable(data, col.names = gsub("[.]", " ", names(data)))
write.csv(data, here("results/covariates", "WM.Cov.age.ed.GA.SIMD.table.csv"), row.names = FALSE)
```

According to above, no variable met the definition of confounder and was thereby used as a covariate in further analyses.

```{r}
markdownToHTML("MRI_Attachment_WM_cov.Rmd",output ="MRI_Attachment_WM_cov.html")
```
